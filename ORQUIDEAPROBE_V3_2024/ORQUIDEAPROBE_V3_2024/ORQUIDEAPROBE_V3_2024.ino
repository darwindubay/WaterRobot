/**************************************************************************
 EJEMPLO TOMADO DE PULOWI MODIFICADO PARA ORQUIEDEA EL 12/05/2024, SE QUITO EL SCROLL CAUSABA
 COSAS RARAS EN LA PANTALLA, INESTABLE PARA LAS NUEVAS.
...
float HumedadP = 0, temperatureC = 0, temperatureA = 0;
...
void setup() {
  ...
  tempsensor.begin();
  //pinMode(A3, OUTPUT);    ///Para reset de pantalalla NOOO PARA ENCENDER PANTALLA. NO APLICA PARA EL CASO DE I2C
...
void escribaD(int sensornumber) {
  ...
  switch (sensornumber) {
    ...
    case 4:
      ...
      //display.print(temperatureA);
      //memcpy(&sensor_data[0], temperatureA, 2);
      //sensor_data = temperatureA;
...
////////////////////////////////////////////TODO///////////////////////////////////////////////////
void loop() {
  //display.display();  //CON ESTO PARECE COMO SI TUVIERA UN VIRUS JAJAJAJA
  if (estado != 0) {
  ...
  if (serial_event == true) {  //if a command was sent to the EZO device.
    ...
    switch (computerdata[0]) {
      ...
      case '4':
        ...
        temperatureA = tempsensor.getTempCByIndex(0);
        escribaD(4);
        if (temperatureA <= 0) {
          pito(250);
        } else {
  ...
  if (digitalRead(PD5) == 1)  //si se oprime el boton
  {
    ...
    escribaD(4);
    delay(1000);
    display.display();  //escribaD(4);
    delay(2000);
    //memcpy(&computerdata[0], "5r", 3);
    //lectura(orp_dir);
    //escribaD(5);  //PARA LAS VARIABLES DE SUELO----- FALTA PARA JEISON RETO
    //delay(3000);
    //escribaD(7);
    //delay(3000);
    //escribaD(8);
